...
function loadDiv(pizzaPlace, pizzaKey) {
    loadMenuJSON(function (response) {
        ...
        const cartSummery = `
                            ...
                            <div class="text-dark">
                                <p class="d-none" id="companyName">${pizzaPlace}</p>
                                <h4>Item:  ${pizza_item.title}<h4>
                                <h4>Price: ${pizza_item.price}</h4>
                                <div class="form-inline">
                                    <label for="quantity"><h4>Quantity:</h4></label>
                                    <input class="form-control w-25 ml-4" type="number" value="1" name="quantity" id="quantity"/><br>
                                    <button class="ml-4 btn btn-success" onclick="checkThis(${perCost})">$</button>    
                                </div>
                                <hr>
                            ...
                            <p class="text-dark" id="address_spot">${sendPublicKey()}</p>
        `;
